...
int main(int argc, char** argv)
{
  ...
  for (int i = 0; i < N; ++i) {
    ...
    digit number = inputs[i];

    bit64_t input_i;
    input_i(number.length()-1, 0) = number(number.length()-1, 0);
  ...
  for (int i = 0; i < N; ++i) {
    ...
    // convert int32 to fixed point
    bit32_t output_i = output;
    interpreted_digit(interpreted_digit.length()-1,0) = output_i(interpreted_digit.length()-1,0);
    out_array[i] = interpreted_digit; 
  }
